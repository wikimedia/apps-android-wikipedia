...
import org.wikipedia.editing.EditClient.Callback;
import org.wikipedia.editing.EditClient.Service;
...
public class EditUnitTest extends MockWebServerTest {
    ...
    private void assertAbuseFilterEditResult(@NonNull Call<Edit> call,
                                       @NonNull Callback cb) {
        verify(cb).success(eq(call), isA(AbuseFilterEditResult.class));
    ...
    private void assertExpectedEditError(@NonNull Call<Edit> call,
        ...
        //noinspection unchecked
        verify(cb, never()).success(any(Call.class), any(SuccessEditResult.class));
