...
class SuggestedEditsImageTagDialog : DialogFragment() {
    ...
    private fun applyResults(results: List<MwQueryPage.ImageLabel>) {
        ...
        if (currentSearchTerm.isEmpty()) {
            ...
            imageTagsRecycler.isVisible = false
            imageTagsDivider.visibility = View.INVISIBLE
        } else {
            imageTagsDivider.isVisible = true
            noResultsText.isVisible = results.isEmpty()
            imageTagsRecycler.isVisible = results.isNotEmpty()
        }
    }
    ...
    companion object {
        fun newInstance(useClipboardText: Boolean, lastText: String): SuggestedEditsImageTagDialog {
            return SuggestedEditsImageTagDialog().apply {
                arguments = bundleOf("useClipboardText" to useClipboardText, "lastText" to lastText)
            }
        }
    }
}
