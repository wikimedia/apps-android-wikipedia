                ...
                if (prefixSearch) {
                    if (actualSearchTerm.length >= 2 && invokeSource != Constants.InvokeSource.PLACES) {
                        withContext(Dispatchers.IO) {
                            listOf(async {
                                getSearchResultsFromTabs(wikiSite, actualSearchTerm)
                            }, async {
                                AppDatabase.instance.historyEntryWithImageDao().findHistoryItem(wikiSite, actualSearchTerm)
                            }, async {
                                AppDatabase.instance.readingListPageDao().findPageForSearchQueryInAnyList(wikiSite, actualSearchTerm)
                            }).awaitAll().forEach {
