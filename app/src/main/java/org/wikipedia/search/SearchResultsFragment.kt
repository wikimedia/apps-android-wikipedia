...
class SearchResultsFragment : Fragment() {
    ...
    private fun doTitlePrefixSearch(searchTerm: String?, force: Boolean) {
        ...
        disposables.add(Observable.timer(if (force) 0 else DELAY_MILLIS.toLong(), TimeUnit.MILLISECONDS).flatMap {
            Observable.zip(ServiceFactory.get(WikiSite.forLanguageCode(searchLanguageCode)).prefixSearch(searchTerm, BATCH_SIZE, searchTerm),
                    if (searchTerm!!.length >= 2) Observable.fromCallable { ReadingListDbHelper.instance().findPageForSearchQueryInAnyList(currentSearchTerm!!) }
                    else Observable.just(SearchResults()),
                    if (searchTerm.length >= 2) Observable.fromCallable { findHistoryItem(currentSearchTerm!!) }
                    else Observable.just(SearchResults()),
                    { searchResponse: PrefixSearchResponse?, readingListSearchResults: SearchResults?, historySearchResults: SearchResults ->
                        val searchResults: SearchResults = if (searchResponse?.query() != null && searchResponse.query()!!.pages() != null) {
    ...
    private fun doSearchResultsCountObservable(searchTerm: String?): Observable<Int> {
        return Observable.fromIterable(WikipediaApp.getInstance().language().appLanguageCodes)
                ...
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .map { response: MwQueryResponse -> if (response.query() != null && response.query()!!.pages() != null) response.query()!!.pages()!!.size else 0 }
    }
    ...
    private fun displayResults(results: List<SearchResult>) {
        for (newResult in results) {
            ...
            for (result in totalResults) {
                if (newResult.pageTitle == result.pageTitle) {
                    contains = true
                    break
                }
            }
    ...
    private inner class SearchResultItemViewHolder(itemView: View) : DefaultViewHolder<View>(itemView) {
        fun bindItem(position: Int) {
            ...
            val result = totalResults[position]
            ...
            if (result.redirectFrom.isNullOrEmpty()) {
                ...
                redirectText.text = getString(R.string.search_redirect_from, result.redirectFrom)
                descriptionText.visibility = View.GONE
            }
            if (result.type == SearchResult.SearchResultType.SEARCH) {
                searchResultIcon.visibility = View.GONE
            } else {
                searchResultIcon.visibility = View.VISIBLE
                searchResultIcon.setImageResource(if (result.type == SearchResult.SearchResultType.HISTORY) R.drawable.ic_history_24 else if (result.type == SearchResult.SearchResultType.TAB_LIST) R.drawable.ic_tab_one_24px else R.drawable.ic_bookmark_white_24dp)
            }

            // highlight search term within the text
            boldenKeywordText(pageTitleText, result.pageTitle.displayText, currentSearchTerm)
            searchResultItemImage.visibility = if (result.pageTitle.thumbUrl == null) if (result.type == SearchResult.SearchResultType.SEARCH) View.GONE else View.INVISIBLE else View.VISIBLE
            loadImageWithRoundedCorners(searchResultItemImage, result.pageTitle.thumbUrl)
            ...
            view.setOnCreateContextMenuListener(LongPressHandler(view,
                    result.pageTitle, HistoryEntry.SOURCE_SEARCH, SearchResultsFragmentLongPressHandler(position)))
        }
    }
