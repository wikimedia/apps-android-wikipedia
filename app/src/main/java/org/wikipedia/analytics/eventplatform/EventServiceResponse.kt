...
/**
 * Moshi POJO representing a response body from the Event Platform's intake API (EventGate).
 *
...
class EventServiceResponse(
    @Json(name = "invalid") val invalidEvents: List<Any> = emptyList(),
    @Json(name = "error") val errorEvents: List<Any> = emptyList()
)
