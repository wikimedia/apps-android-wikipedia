...
interface HistoryEntryDao {
    ...
    @Query("SELECT CAST(strftime('%H', (timestamp + :utcOffsetMillis) / 1000, 'unixepoch') AS INTEGER) AS hour FROM HistoryEntry WHERE timestamp BETWEEN :startDate AND :endDate GROUP BY hour ORDER BY COUNT(id) DESC LIMIT 1")
    suspend fun getFavoriteTimeToReadSince(startDate: Long, endDate: Long, utcOffsetMillis: Int): Int?

    @Query("SELECT CAST(strftime('%w', (timestamp + :utcOffsetMillis) / 1000, 'unixepoch') AS INTEGER) AS dayOfWeek FROM HistoryEntry WHERE timestamp BETWEEN :startDate AND :endDate GROUP BY dayOfWeek ORDER BY COUNT(id) DESC LIMIT 1")
    suspend fun getFavoriteDayToReadSince(startDate: Long, endDate: Long, utcOffsetMillis: Int): Int?

    @Query("SELECT CAST(strftime('%m', (timestamp + :utcOffsetMillis) / 1000, 'unixepoch') AS INTEGER) AS month FROM HistoryEntry WHERE timestamp BETWEEN :startDate AND :endDate GROUP BY month ORDER BY COUNT(id) DESC LIMIT 1")
    suspend fun getMostReadingMonthSince(startDate: Long, endDate: Long, utcOffsetMillis: Int): Int?
