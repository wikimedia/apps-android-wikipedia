...
interface HistoryEntryDao {
    ...
    @Query("SELECT displayTitle FROM HistoryEntry WHERE timestamp BETWEEN :startMillis AND :endMillis GROUP BY displayTitle ORDER BY COUNT(displayTitle) DESC LIMIT :limit")
