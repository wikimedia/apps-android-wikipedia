...
object DeviceUtil {
    ...
    fun updateStatusBarTheme(activity: Activity, toolbar: Toolbar?, reset: Boolean) {
        ...
        if (toolbar != null) {
            toolbar.navigationIcon!!.setColorFilter(if (reset) activity.resources.getColor(android.R.color.white) else ResourceUtil.getThemedColor(activity, R.attr.toolbar_icon_color), PorterDuff.Mode.SRC_IN)
        }
    }
    ...
    private val connectivityManager: ConnectivityManager
        get() = WikipediaApp.getInstance().getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
