...
class LinkPreviewBottomSheet(context: Context, attrs: AttributeSet? = null) : CoordinatorLayout(context, attrs), LinkPreviewErrorView.Callback {
    ...
    private var loadPageCallback: LoadPageCallback? = null
    ...
    private val menuListener = PopupMenu.OnMenuItemClickListener { item ->
        return@OnMenuItemClickListener when (item.itemId) {
            ...
            R.id.menu_link_preview_share_page -> {
                ShareUtil.shareText(activity, viewModel.pageTitle)
                true
            }
            ...
            R.id.menu_link_preview_copy_link -> {
                ...
                ClipboardUtil.setPlainText(activity, text = viewModel.pageTitle.uri)
            ...
            R.id.menu_link_preview_view_on_map -> {
                ...
                viewModel.location?.let {
                   activity.startActivity(PlacesActivity.newIntent(activity, viewModel.pageTitle, it))
                }
                // TODO: dismiss()
                true
            }
            R.id.menu_link_preview_get_directions -> {
                ...
                viewModel.location?.let {
                    GeoUtil.sendGeoIntent(activity, it, StringUtil.fromHtml(viewModel.pageTitle.displayText).toString())
                }
                true
            }
            else -> false
    ...
    private val galleryViewListener = GalleryViewListener { view, thumbUrl, imageName ->
        ...
        view.drawable?.let {
            ...
            view.transitionName = activity.getString(R.string.transition_page_gallery)
            options = ActivityOptionsCompat.makeSceneTransitionAnimation(activity, view, activity.getString(R.string.transition_page_gallery))
        }
        requestGalleryLauncher.launch(GalleryActivity.newIntent(activity, viewModel.pageTitle,
            imageName, viewModel.pageTitle.wikiSite, revision, GalleryActivity.SOURCE_LINK_PREVIEW), options)
    }

    private val requestGalleryLauncher = activity.registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        if (it.resultCode == GalleryActivity.ACTIVITY_RESULT_PAGE_SELECTED && it.data != null) {
            activity.startActivity(it.data!!)
        }
    }

    init {
        _binding = DialogLinkPreviewBinding.inflate(activity.layoutInflater)
                ...
                viewModel.uiState.collect {
                    when (it) {
                        ...
                        is LinkPreviewViewState.Watch -> {
                            showWatchlistSnackbar(activity, viewModel.pageTitle)
                            // TODO: dismiss()
                        }
        ...
        if (overlayView == null) {
            LinkPreviewOverlayView(activity).let {
                overlayView = it
    ...
    private fun setupOverflowMenu() {
        val popupMenu = PopupMenu(activity, binding.linkPreviewOverflowButton)
        ...
        popupMenu.menu.findItem(R.id.menu_link_preview_watch).title = activity.getString(if (viewModel.isWatched) R.string.menu_page_unwatch else R.string.menu_page_watch)
    ...
    private fun showWatchlistSnackbar(activity: AppCompatActivity, pageTitle: PageTitle) {
        viewModel.pageTitle.let {
            if (!viewModel.isWatched) {
                FeedbackUtil.showMessage(activity, activity.getString(R.string.watchlist_page_removed_from_watchlist_snackbar, it.displayText))
            } else if (viewModel.isWatched) {
                val snackbar = FeedbackUtil.makeSnackbar(activity,
                    activity.getString(R.string.watchlist_page_add_to_watchlist_snackbar, it.displayText, activity.getString(WatchlistExpiry.NEVER.stringId)))
    ...
    private fun doAddToList() {
        ReadingListBehaviorsUtil.addToDefaultList(activity, viewModel.pageTitle, true, Constants.InvokeSource.LINK_PREVIEW_MENU)
        // TODO: dismiss()
    }
        ...
        if (viewModel.isInReadingList) {
            LongPressMenu(anchorView, existsInAnyList = false, callback = object : LongPressMenu.Callback {
                override fun onAddRequest(entry: HistoryEntry, addToDefault: Boolean) {
                    ReadingListBehaviorsUtil.addToDefaultList(activity, viewModel.pageTitle, addToDefault, Constants.InvokeSource.LINK_PREVIEW_MENU)
                    // TODO: dismiss()
                }

                override fun onMoveRequest(page: ReadingListPage?, entry: HistoryEntry) {
                    page?.let { readingListPage ->
                        ReadingListBehaviorsUtil.moveToList(activity, readingListPage.listId, viewModel.pageTitle, Constants.InvokeSource.LINK_PREVIEW_MENU)
                    }
                    // TODO: dismiss()
                }
    ...
    private fun setPreviewContents(contents: LinkPreviewContents) {
        if (!contents.extract.isNullOrEmpty()) {
            ...
            val colorHex = ResourceUtil.colorToCssString(ResourceUtil.getThemedColor(activity, android.R.attr.textColorPrimary))
        ...
        loadPageCallback.let {
            if (it != null) {
                ...
                activity.startActivity(
                    if (inNewTab) PageActivity.newIntentForNewTab(activity, entry, entry.title)
                    else PageActivity.newIntentForCurrentTab(activity, entry, entry.title, false)
                )
            }
        }
    }
