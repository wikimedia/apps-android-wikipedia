...
class EditHistoryListActivity : BaseActivity() {
    ...
    private fun updateCompareStateItems() {
        ...
        selectedRevisionFrom?.let {
            binding.compareFromText.text = DateUtil.getShortDayWithTimeString(it.localDateTime)
        }
        binding.compareToCard.isVisible = selectedRevisionTo != null
        selectedRevisionTo?.let {
            binding.compareToText.text = DateUtil.getShortDayWithTimeString(it.localDateTime)
        }
