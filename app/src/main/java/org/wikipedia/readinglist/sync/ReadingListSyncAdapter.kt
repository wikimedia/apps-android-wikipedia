...
class ReadingListSyncAdapter : AbstractThreadedSyncAdapter {
                    ...
                    for (i in localPages.indices) {
                        ...
                        try {
                            ...
                            if (client.isErrorType(t, "duplicate-page")) {
                                ...
                                // ...and also log it:
                                L.logRemoteError(RuntimeException("Attempted to sync malformed page: ${localPage.wiki()}, ${localPage.title()}"))
                            } else {
                                throw t
                            }
                        }
    ...
    private fun getCsrfToken(wiki: WikiSite, tokenList: MutableList<String>): String {
        if (tokenList.size == 0) {
            tokenList.add(CsrfTokenClient(wiki, wiki).token.blockingSingle())
        }
        return tokenList[0]
    }
