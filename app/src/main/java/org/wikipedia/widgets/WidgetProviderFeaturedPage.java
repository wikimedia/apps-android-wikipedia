...
public class WidgetProviderFeaturedPage extends AppWidgetProvider {
    ...
    public void onUpdate(final Context context, final AppWidgetManager appWidgetManager,
        ...
        getFeaturedArticleInformation(
                (final PageTitle pageTitle, final CharSequence widgetText) -> {
                    for (final int widgetId : allWidgetIds) {
                        ...
                        // If we want to update the widget itself from the click event, then do
                        // something like this:
                        //Intent intent = new Intent(context, WidgetProviderFeaturedPage.class);
                        //intent.setAction(AppWidgetManager.ACTION_APPWIDGET_UPDATE);
    ...
    private void getFeaturedArticleInformation(final Callback cb) {
        ...
        ServiceFactory.getRest(WikipediaApp.getInstance().getWikiSite()).getAggregatedFeed(
                date.year(), date.month(), date.date())
                .flatMap(response -> {
                    if (response.tfa() != null) {
                        return Observable.just(response.tfa());
                    } else {
                        // TODO: this logic can be removed if the feed API can return the
                        // featured article for all languages.
                        return getApiService(mainPageTitle)
                                .lead(mainPageTitle.getWikiSite(), null, null, null,
                                        mainPageTitle.getPrefixedText(),
                ...
                .subscribe(response -> {
                    ...
                    if (response instanceof retrofit2.Response) {
                        ...
                        widgetText = StringUtil.fromHtml(
                                ((RbPageSummary) response).getDisplayTitle());
    ...
    private interface Callback {
        void onFeaturedArticleReceived(@NonNull PageTitle pageTitle,
                @NonNull CharSequence widgetText);
    }
}
