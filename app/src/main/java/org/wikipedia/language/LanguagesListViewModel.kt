...
class LanguagesListViewModel : ViewModel() {
    ...
    private val suggestedLanguageCodes = WikipediaApp.instance.appLanguageState.remainingAvailableLanguageCodes
    private val nonSuggestedLanguageCodes = WikipediaApp.instance.appLanguageState
        .appMruLanguageCodes.toMutableList().also {
                it.removeAll(suggestedLanguageCodes)
                it.removeAll(WikipediaApp.instance.appLanguageState.appLanguageCodes)
            }
    ...
    private fun addFilteredLanguageListItems(filter: String, codes: List<String>, headerText: String,
        ...
        var first = true
        for (code in codes) {
            val localizedName = StringUtils.stripAccents(WikipediaApp.instance.appLanguageState.getAppLanguageLocalizedName(code).orEmpty())
    ...
    fun getCanonicalName(code: String): String? {
        return siteListData.value?.data?.find { it.code == code }?.localname.orEmpty()
                .ifEmpty { WikipediaApp.instance.appLanguageState.getAppLanguageCanonicalName(code) }
    }
