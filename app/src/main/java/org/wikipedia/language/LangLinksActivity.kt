...
class LangLinksActivity : BaseActivity() {
    ...
    private val entriesByAppLanguages: List<PageTitle>
        get() = languageEntries.filter {
            it.wikiSite.languageCode() == AppLanguageLookUpTable.NORWEGIAN_LEGACY_LANGUAGE_CODE &&
                    app.appLanguageState.appLanguageCodes.contains(AppLanguageLookUpTable.NORWEGIAN_BOKMAL_LANGUAGE_CODE) ||
                    app.appLanguageState.appLanguageCodes.contains(it.wikiSite.languageCode())
        }
    ...
    private fun updateLanguageEntriesSupported(languageEntries: MutableList<PageTitle>) {
        ...
        while (it.hasNext()) {
            ...
            val languageVariants = app.appLanguageState.getLanguageVariants(languageCode)
        ...
        addVariantEntriesIfNeeded(app.appLanguageState, title, languageEntries)
    }

    private fun updateVariantDisplayTitleIfNeeded() {
        val list = languageEntries.filter { !app.appLanguageState.getDefaultLanguageCode(it.wikiSite.languageCode()).isNullOrEmpty() }
    ...
    private fun sortLanguageEntriesByMru(entries: MutableList<PageTitle>) {
        var addIndex = 0
        for (language in app.appLanguageState.mruLanguageCodes) {
    ...
    private inner class LangLinksAdapter(languageEntries: List<PageTitle>, private val appLanguageEntries: List<PageTitle>) : RecyclerView.Adapter<DefaultViewHolder>() {
        ...
        fun setFilterText(filterText: String) {
            ...
            for (entry in originalLanguageEntries) {
                ...
                val canonicalName = app.appLanguageState.getAppLanguageCanonicalName(languageCode).orEmpty()
                val localizedName = app.appLanguageState.getAppLanguageLocalizedName(languageCode).orEmpty()
    ...
    private inner class LangLinksItemViewHolder constructor(private val languageEntries: List<PageTitle>, itemView: View) : DefaultViewHolder(languageEntries, itemView), View.OnClickListener {
        ...
        override fun bindItem(position: Int) {
            ...
            val localizedLanguageName = app.appLanguageState.getAppLanguageLocalizedName(languageCode)
            ...
            if (binding.langlinksLoadProgress.visibility != View.VISIBLE) {
                ...
                if (canonicalName.isNullOrEmpty() || languageCode == app.appLanguageState.systemLanguageCode) {
        ...
        override fun onClick(v: View) {
            ...
            app.appLanguageState.addMruLanguageCode(langLink.wikiSite.languageCode())
